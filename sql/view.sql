...
CREATE OR REPLACE VIEW `v_visit_month` as select count(id) total , m as month from (select id , date_format(create_time , '%Y-%m') m from sys_log) t group by t.m order by m desc;
